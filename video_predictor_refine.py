...
for out_frame_idx in range(ann_frame_idx, len(video_dir), 1):
    masks = {}
...
# plt.figure(figsize=(12, 8))
# plt.title(f"frame {refine_frame_idx} -- before refinement")
# plt.imshow(Image.open(video_dir[refine_frame_idx]))
# show_mask(video_segments[refine_frame_idx][ann_obj_id], plt.gca(), obj_id=ann_obj_id)
# points = []
# labels = []
...
#             )
#             ax.clear()
#             img = Image.open(video_dir[refine_frame_idx])
#             ax.imshow(img)
#             show_mask((out_mask_logits[0] > 0.0).cpu().numpy(), ax, obj_id=out_obj_ids[0])
#             show_points(points_np, labels_np, ax)
#             plt.draw()
# # # Load and display the image
# fig, ax = plt.subplots(figsize=(12, 8))
# plt.title(f"frame {refine_frame_idx}")
# ax.imshow(Image.open(video_dir[refine_frame_idx]))
